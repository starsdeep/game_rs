...
class Server():
    ...
    def inputGame(self):
        ...
        try:
            ...
            self.title_pkg_dict = {item[1].strip():item[0] for item in items if len(item[0].strip()) > 0 and len(item[1].strip())>0 }            
            ...
            print "pkgname for 鬼屋魅影2:" + self.title_pkg_dict['鬼屋魅影2'.decode('utf-8')]
    ...
    def buildResponse(self,type,query):
        ...
        #bagOfwords
        response+='\003'
        for word,weight in self.per_pkg_word_weight_dict[pkgname].items():
            response += word + ":" + str("%.2f"%weight) +'   ' 
        response+='\001'
        #build top 50
        num=0
        if pkgname in self.topSim_dict:
            ...
            for other,sim in self.topSim_dict[pkgname].items():
                ...
                if other in self.per_pkg_word_weight_dict:
                    for word,weight in self.per_pkg_word_weight_dict[other].items():
                        response += word + ":" + str("%.2f"%weight) +'   ' 
                #tag sim
                response += '\003'
                if pkgname in self.tagSim_dict and other in self.tagSim_dict[pkgname]:
                    response+=str("%.2f"%self.tagSim_dict[pkgname][other])+'\003'
                else:
                    response+='None'+'\003'
                #desp sim
                if pkgname in self.despSim_dict and other in self.despSim_dict[pkgname]:
                    response+=str("%.2f"%self.despSim_dict[pkgname][other])+'\003'
                else:
                    response+='None'+'\003'
                #top sim 
                if pkgname in self.topSim_dict and other in self.topSim_dict[pkgname]:
                    response+=str("%.2f"%self.topSim_dict[pkgname][other])
                response+='\002'
    ...
    def loadData(self):
        print 'load data'     
        #load data
        f = file('topSim_dict.data')
        self.topSim_dict = p.load(f)
        f.close()

        f = file('tagSim_dict.data')
        self.tagSim_dict = p.load(f)
        f.close()
    
        f = file('despSim_dict.data')
        self.despSim_dict = p.load(f)
        f.close()
        
        f = file('per_pkg_word_weight_dict.data')
        self.per_pkg_word_weight_dict = p.load(f)
        f.close()
        print "first testing"
        #print "desp sim for com.furiousapps.haunt2 and com.rexsee.fch.spm6l2:" + str(self.despSim_dict['com.furiousapps.haunt2']['com.rexsee.fch.spm6l2'])
