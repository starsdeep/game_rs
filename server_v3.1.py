...
class Server():
    ...
    def buildWordPkgSet(self):
        self.word_pkgSet_dict = LanguageModel.getInvertedIndex_list(self.pkg_segtitle_dict)
    ...
    def loadData(self):
        print 'load data'     
        self.topSim_dict = WriteTool.load_nested_dict('data/discounted_sim')
        print str(self.topSim_dict['com.imangi.templerun2']['com.notdoppler.earntodie'])
        
        
        f = file('./data/pkg_tfidf_dict.data')
        self.per_pkg_word_weight_dict= p.load(f)
        f.close()

        f = file('./data/pkg_segtitle_dict.data')
        self.pkg_segtitle_dict = p.load(f)
        for title in self.pkg_segtitle_dict['com.gameon.bubblebust']:
            print title
        f.close()
